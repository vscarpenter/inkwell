...
The 14 destination example pages and `preview.html` carry a runtime palette toggle (widget styled in `examples/demo.css`, wired in `examples/demo.js`). It reads `?palette=clay` from the URL on load, persists to `localStorage("inkwell-palette")`, and dynamically swaps a `<link>` element in `<head>`. `examples/index.html` carries the toggle too (it already rendered the saved palette via the pre-paint, and the live site lands there — the palette story should be demoable at first touch); only the root `index.html` (the minimal starter template) skips it, to stay copy-paste minimal. Every destination page also carries a hand-copied `<head>` pre-paint snippet (theme attribute + palette `<link>`); when changing it, update all copies — they are marked KEEP IN SYNC.
